...
#     ./mavgen.py --lang=C --wire-protocol=$protocol --output=C/include_v$protocol $xml || exit 1
     ./mavgen.py --lang=Go --wire-protocol=$protocol --output=Go/include_v$protocol $xml || exit 1
#     mkdir -p python
#     ./mavgen.py --lang=Python --wire-protocol=$protocol --output=python/mavlink_${base}_v$protocol.py $xml || exit 1
#     mkdir -p go
#     ./mavgen.py --lang=Go --wire-protocol=$protocol --output=go/mavlink_${base}_v$protocol.go $xml || exit 1
 done
done
